...
Thirteen checks the user runs after install (or whenever something feels off); `C1` and `C6` are legacy and return `SKIP`. The active eleven validate that the hook commands in `~/.claude/settings.json` resolve to existing scripts (`C2`), that the JSON schemas compile (`C3`), that the project's state directory is initialized (`C4`), that the hook error log shows no recent recurring errors (`C5`), that the runtime's rate-limit feed is wired up and freshly captured (`C7`), that slash commands are not duplicated across `~/.claude/commands/` and `~/.claude/commands/metaensemble/` (`C8`), that the vendored runtime at `~/.metaensemble/runtime/` is a valid symlink into a version dir with a verifying MANIFEST and an executable runner (`C9`), that recent Runs recorded without failure and no pending sidecars linger (`C10`), that no duplicate files sit across the MetaEnsemble catalogs (`C11`), that the pinned interpreter's install topology is sound (`C12`), and that no quarantined or stale pending sidecars need attention (`C13`). The `--fix` flag applies safe remediations.
