...
Concrete use cases the v0.1.0 release supports:
...
- Surface-level dispatch protocols: solo, fanout, consensus, peer
  review. v0.1.0 validates planning, markers, and guardrails for these
...
- Production-scale multi-tenant workloads. The Ledger is SQLite on the
  ...
  project is not supported in v0.1.0.
- Domain-specific classification or scoring where calibrated confidence is
  ...
  record such work, but the v0.1.0 eval harness does not certify domain
  accuracy.
...
Not yet at the standard required for a product claim. v0.1.0 has strong
...
domain variety to support such a catalog in v0.1.0.
...
would close the remaining gap; the v0.1.0 release does not include
