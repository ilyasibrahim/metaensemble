...
Honesty up front: MetaEnsemble is **a layer of conventions, schemas, hooks, and skills** built on the agent runtime's existing primitives — Tasks, hooks, skills, slash commands, output styles. It is not a new daemon, a new network protocol, or a literal service mesh.
...
| PreToolUse (Task / Agent) | `pre_task.py` | Resolve target Manifest. Validate against schema. Run cost gate (see §9). Block if over hard threshold; on NOTIFY *or* BLOCK write structured options to `state/notifies/<session>-<ts>.json` or `state/blocks/<session>-<ts>.json` so the Coordinator can surface them to the Principal — the runtime renders a BLOCK as a generic error and even a NOTIFY benefits from a stable place to recover the options from. |
...
| PostToolUse (Task / Agent) | `post_task.py` | Complete Run record from sidecar. Append Run to SQLite + JSONL with classified outcome (`ok` / `failed` / `partial`) and, when failed, a `failure_reason` label. The label is one of the categorical values `metaensemble/lib/recording.py:classify_failure_reason` emits (`cost_gate_block` / `manifest_invalid` / `timeout` / `exception` / `other`); the same column also carries free-form strings written by the reconcile module on `interrupted` / `budget_exceeded` outcomes (see §8.1). Run the Python deliverable check (see §10) on successful Runs whose Manifest declared Python deliverables; persist `quality_state` and `quality_findings_json` on the Run row, and (when transcript walking succeeded) the 003 provenance columns `requested_model_tier`, `model_source`, `deliverable_ref_json`, `files_touched_json`, `tool_use_json`, `review_findings_json`, `cache_read_tokens`, `cache_create_tokens`. Update Executor `last_seen_ts`. |
...
| PostToolUse (Write) | `deliverable_sync.py` | If a Deliverable was written under `reports/`, register its path in the deliverables index. |
| Stop | `session_summary.py` | Render session digest: Executors active, Runs completed, window % consumed. |
...
- **Layer 1 — Stop hook.** `session_summary.py` reconciles every sidecar whose `session_id` matches the ending session. Catches Ctrl-C and graceful exits where Stop still fires.
...
## 10. Python deliverable check — output evaluation after a Run

The cost gate addresses the dimension of consequence — what is *about* to happen. The Python deliverable check addresses a narrower dimension of output — what has *just* happened for successful Runs whose Manifest declares Python deliverables. It reuses the quality-gate implementation names in code for compatibility, but v0.1.0 documentation should be read literally: this is an opt-in Python check, not a universal quality judge. Both gates share the same three-state grammar (AUTO / NOTIFY / BLOCK), the worst-of-axes aggregation, and the structured-options Principal surface. The gates run at different moments because the data they need lands at different moments: cost is a property of the intended Run and is knowable PreToolUse; deliverable quality signals are only knowable PostToolUse.
