...
**First full-tier calibration cycle** (closes issue #1). 320 live runs — 8 cells (B1–B4 baselines, `MM_full`, three ablations) × 8 Suite-A software tasks × 5 seeds — at a USD 0.60/run cap with `claude-sonnet-5` as executor; observed cost USD 140.55; report checked in at `evals/reports/20260704T140844Z-full.md`. Measured: acceptance-quality parity between `MM_full` and strong single-agent baselines at a 1.55× token premium on single-context tasks (D-8 PASS at 1.55× ≤ 2.0×; D-9 FAIL at 62.9% waste — the superiority claim stays unclaimable per the project's own discipline); every protocol primitive is load-bearing (ablating the Manifest collapses pass@budget 0.400 → 0.075); the full protocol more than doubles the runtime's default-subagent baseline (0.400 vs 0.175). SYSTEM-CARD, README, PERFORMANCE §4, RELEASE-CHECKLIST claim discipline, and the `metaensemble init` notice all updated to the measured statements.
